...
public class EmployeeDAO {
    ...
    public boolean recordExists(String nationalID) {
        try {
            String query = "SELECT * FROM hrh.emp_bio WHERE national_id = ?";
    ...
    public List<Employee> findAll() {
        ...
        try {
            String sql = "SELECT e.id as emp_id,IFNULL(e.emp_no,'-') as emp_no,  IFNULL(e.first_name,'') as firstname,\n"
                    + "IFNULL(e.surname,'') as surname, IFNULL( e.other_name,'') as other_name,IFNULL( st.name,'') as emp_status,CONCAT(IFNULL(e.first_name,'') , ' ',\n"
                    + "IFNULL( e.other_name,''), ' ',  IFNULL(e.surname,' ')) as full_name,IFNULL(e.phone,'_') as phone, IFNULL(e.email,'') as email,IFNUll(e.nationality,'-') as nationality,\n"
                    + "IFNULL(h1.position_id,h2.position_id) as current_position_id, COALESCE(p1.position_title,p2.position_title) as current_position\n"
                    + "FROM hrh.emp_bio e\n"
                    + "LEFT JOIN hrh.employee_hist h1 ON e.emp_no = h1.emp_no AND h1.date_ended IS NULL\n"
                    + "LEFT JOIN cadre_positions p1 ON h1.position_id = p1.id\n"
                    + "LEFT JOIN (SELECT emp_no, position_id,isActive FROM hrh.employee_hist WHERE date_ended IS NOT NULL ORDER BY date_ended DESC) h2 ON e.emp_no = h2.emp_no \n"
                    + "LEFT JOIN cadre_positions p2 ON h2.position_id = p2.id\n"
                    + "LEFT JOIN employee_status st ON e.status = st.id\n"
                    + "GROUP BY e.emp_no";
    ...
    public Employee getEmployeeDetails(String emp_no) throws SQLException {
...
//        conn.rs = conn.st.executeQuery();
        ...
        // Set the employment history list of the employee
//        employee.setEmploymentHistoryList(employmentHistoryList);
