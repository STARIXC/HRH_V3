...
public class LeaveTypeController extends HttpServlet {
    ...
    public int status, execute_activity, accrual_rate = 0;
    private final LeaveTypeDAO dao;
    Gson gson = new Gson();
...
//            System.out.println(leave_id);
        ...
        } else if (action.equalsIgnoreCase("save_leave_type")) {
//            leave.setLeave_id(lid);
            ...
            String leave_ = JSONConverter.convert(dao.getLeaveById(leave_id));
            out.println(leave_);
        } else if (action.equalsIgnoreCase("getAllTypes")) {
